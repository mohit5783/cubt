...
<? 	
	...
	$_SESSION['CurrentSubMenu']="Manage-User";
?>
		...
		<div id="content-table-inner">
			...
			<div id="table-content">
				...
				<tr>
					...
					<th class="table-header-repeat line-left minwidth-1"><a href="">SSD Login ID</a></th>
				...
				/*############### Putting all the UserType in the Array So that ################### 
				  ############### the calls to the database can be minimized.   ###################*/
					...
					{
						$UserTypeArray[$rs['UserTypeID']]= $rs['UserTypeName'];
					}
				/*############### altrow is for the alternative row class finder ################### */
					$altrow=false;
					$newque="SELECT * FROM User";
					$qu=mysql_query($newque);
					while($rs=mysql_fetch_array($qu))
					{
				?>
				<tr <? if($altrow){echo "class='alternate-row'";} ?> >
					...
					<td><?=$rs['FullName']?></td>
					<td><?=$rs['SSDLoginID']?></td>
					<td><a href=""><?=$rs['Email']?></a></td>
					<td><?=$rs['Phone']?></td>
					<td><a href=""><?=$UserTypeArray[$rs['UserTypeID']]?></a></td>
